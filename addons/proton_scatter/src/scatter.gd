...
# Creates collision data with the Physics server directly.
# HACK : Modifié pour forcer un cylindre de tronc sur chaque arbre !
func _create_collision(body: StaticBody3D, t: Transform3D) -> void:
	...
	# --- DEBUT DU HACK : Création forcée d'un cylindre ---
	var shape_rid = PhysicsServer3D.cylinder_shape_create()
	
	# ⚠️ MODIFIE CES DEUX VALEURS SELON LA TAILLE DE TON ARBRE ⚠️
	var rayon_tronc = 0.2 
	var hauteur_tronc = 4.0 
	
	var data = {
		"radius": rayon_tronc,
		"height": hauteur_tronc,
	}
	PhysicsServer3D.shape_set_data(shape_rid, data)
	
	# Le centre d'un cylindre Godot est à son milieu. 
	# On le remonte de la moitié de sa hauteur pour qu'il pose sur le sol.
	var transform_collision = t
	transform_collision.origin.y += (hauteur_tronc / 2.0)
	
	# On attache le cylindre au corps physique
	PhysicsServer3D.body_add_shape(_body_rid, shape_rid, transform_collision)
